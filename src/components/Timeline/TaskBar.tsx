...
import { type TimelineTask,type TaskPosition } from '../../types/timeline.types';
...
export const TaskBar: React.FC<TaskBarProps> = memo(({
  ...
  const ariaLabel = `${task.title}. From ${formatDate(task.startDate)} to ${formatDate(task.endDate)}. Progress: ${task.progress}%. ${task.assignee ? `Assigned to ${task.assignee}.` : ''} Press Enter to edit.`;
    ...
    return (
      <div
        ...
        style={{
          ...
          top: `${position.top}px`,
          zIndex: isDragging ? TIMELINE_CONSTANTS.Z_INDEX.DRAGGING : TIMELINE_CONSTANTS.Z_INDEX.TASK,
        }}
        onClick={handleClick}
        onMouseDown={(e) => onDragStart(e, task.id, position.left, task.rowId)}
        role="button"
        tabIndex={0}
  ...
  return (
    <div
      className={`absolute rounded shadow-sm cursor-move hover:shadow-lg transition-shadow group ${isDragging ? 'opacity-50' : ''} ${isResizing ? 'cursor-ew-resize' : ''}`}
      style={{
        left: `${position.left}px`,
        ...
        backgroundColor: task.color || TIMELINE_CONSTANTS.DEFAULT_TASK_COLOR,
        zIndex: isDragging || isResizing ? TIMELINE_CONSTANTS.Z_INDEX.DRAGGING : TIMELINE_CONSTANTS.Z_INDEX.TASK,
      }}
      onClick={handleClick}
      onMouseDown={(e) => {
